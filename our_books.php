<?php
  include_once './includes/partials/header.php';
 include_once '../includes/functions/function.php';
?>
<?php
  include_once './includes/partials/nav.php';
?>

<main>
<section class="sec1">
            <div class="rated-books-ctn">
                <div class="sec-title">
                    <span class="title-wrap">
                        <h4 class="text-red">Nos livres</h4>
                    </span>
                    <div></div>
                </div>

                <div class="shelft-ctn">
                    <div class="shelft-carousel">
                        <div class="shelft-carousel-view">
                            <div class="shelft-item">
                                <img src="./assets/img/book.jpeg" alt="">
                            </div>
                            <div class="shelft-item">
                                <img src="./assets/img/book.jpeg" alt="">
                            </div>
                            <div class="shelft-item">
                                <img src="./assets/img/book.jpeg" alt="">
                            </div>
                            <div class="shelft-item">
                                <img src="./assets/img/book.jpeg" alt="">
                            </div>
                        </div>
                       
                    </div>
                    <img src="./assets/img/Sw2.PNG" class="shelft-img" alt="">
                </div>
            </div>

            <div class="rated-books-ctn">
                <div class="sec-title">
                    
                    <div></div>
                </div>

                <div class="shelft-ctn">
                    <div class="shelft-carousel">
                        <div class="shelft-carousel-view">
                            <div class="shelft-item">
                                <img src="./assets/img/book.jpeg" alt="">
                            </div>
                            <div class="shelft-item">
                                <img src="./assets/img/book.jpeg" alt="">
                            </div>
                            <div class="shelft-item">
                                <img src="./assets/img/book.jpeg" alt="">
                            </div>
                            <div class="shelft-item">
                                <img src="./assets/img/book.jpeg" alt="">
                            </div>
                        </div>
                       
                    </div>
                    <img src="./assets/img/Sw2.PNG" class="shelft-img" alt="">
                </div>
            </div>

            <div class="rated-books-ctn">
                <div class="sec-title">
                    
                    <div></div>
                </div>

                <div class="shelft-ctn">
                    <div class="shelft-carousel">
                        <div class="shelft-carousel-view">
                            <div class="shelft-item">
                                <img src="./assets/img/book.jpeg" alt="">
                            </div>
                            <div class="shelft-item">
                                <img src="./assets/img/book.jpeg" alt="">
                            </div>
                            <div class="shelft-item">
                                <img src="./assets/img/book.jpeg" alt="">
                            </div>
                            <div class="shelft-item">
                                <img src="./assets/img/book.jpeg" alt="">
                            </div>
                        </div>
                       
                    </div>
                    <img src="./assets/img/Sw2.PNG" class="shelft-img" alt="">
                </div>
            </div>

            <div class="pagination" style="display: flex; justify-content: center; gap: 20px;">
                    <button type="button" class="more-btn">Début</button>
                    <button type="button" class="more-btn">&laquo;</button>
                    <a href="#" class="more-btn-pagination">1</a>
                    <a href="#" class="active more-btn">2</a>
                    <a href="#" class="more-btn">3</a>
                    <a href="#" class="more-btn">4</a>
                    <a href="#" class="more-btn">5</a>
                    <button type="button" class="more-btn" >&raquo;</button>
                    <button type="button" class="more-btn">Fin</button>
            </div>
            

        </section>

    <?php
        include_once './includes/partials/cate_search_bar.php';
    ?>
</main>





<?php
  include_once './includes/partials/footer.php';
?>


